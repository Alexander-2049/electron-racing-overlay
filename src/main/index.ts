...
import { iRacingSDK } from "./iRacing/iRacingSDK";
...
const createWindow = (): BrowserWindow => {
  ...
  irsdkipc.on("telemetry", (telemetry) => {
    ...
    connections.controls.send({
      ...
      steeringAngle: telemetry.data.SteeringWheelAngle,
      throttle: telemetry.data.Throttle,
    });
    
    connections.speed.send({
      displayUnits: telemetry.data.DisplayUnits === 0 ? "IMPERIAL" : "METRIC",
      speedKph: SpeedConverter.convert(telemetry.data.Speed, "METERS_PER_SECOND", "KILOMETERS_PER_HOUR"),
      speedMph: SpeedConverter.convert(telemetry.data.Speed, "METERS_PER_SECOND", "MILES_PER_HOUR"),
    })
